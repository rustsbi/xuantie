...
//!
//! C906 will invalid all I-cache automatically when reset; use the following operation to enable I-cache.
//! 
//! ```no_run
//! // If we don't have `mxstatus.theadisaee` enabled
//! use xuantie::register::mcor::{self, Cache, Operation};
//! unsafe { 
//!     mcor::cache(Cache::BOTH, Operation::INVALIDATE);
//! }
//! // If we have `mxstatus.theadisaee` enabled
//! use xuantie::asm::{icache_iall, sync_is};
//! unsafe {
//!     icache_iall();
//!     sync_is();
//! }
//! ```
//!
//! Additionally, we can disable I-cache manually if necessary.
//!
//! ```no_run
//! use xuantie::register::mhcr;
//! // enable D-cache
//! unsafe { mhcr::clear_de() };
//! ```
//!
//! ## CPU power down
//!
